...
namespace OpenTKUtils.GL4
{
    ...
    public class GLTesselationShaderSinewaveAutoscaleLookatInstanced : GLShaderPipeline
    {
        ...
        public GLTesselationShaderSinewaveAutoscaleLookatInstanced(float tesselation,float amplitude, float repeats, bool rotate = false, bool rotateelevation = true, bool commontransform = false,
                                                    float autoscale=0, float autoscalemin = 0.1f, float autoscalemax= 3f)
        {
            var vert = new GLPLVertexScaleLookat(rotate,rotateelevation, false, autoscale, autoscalemin, autoscalemax);
