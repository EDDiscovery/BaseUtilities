...
void main(void)
{
	...
	float z = p[minz].z + zdist * (0.99+instanceid * 0.1);		// first is painted at the back, then forward..
    ...
    for ( ic = 0 ; ic < 12 ; ic++ )
    {
		...
		int i2 = i2lookup[ic];		// computes the pairs to compare..

		float interceptv = (z - p[i].z) / (p[i2].z - p[i].z);	// z% intercept on those pairs
